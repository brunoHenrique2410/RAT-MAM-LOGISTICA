...
# CONFIGURAÇÃO BÁSICA
# =========================
PDF_BASE_PATH = "RAT MAM.pdf"  # deixe o PDF base na raiz do repo
APP_TITLE = "RAT MAM - Preenchimento Automático (enxuto / overlay)"
...
st.header("2) Cliente (topo do PDF)")
c1, c2 = st.columns(2)
with c1:
...
atividade = st.text_area(
    "Atividade (palavras do técnico)",
    placeholder="Descreva o que foi feito: troca/configuração/testes/resultados etc.",
    height=120,
)
info_extra = st.text_area("Informações adicionais (opcional)", height=80)

st.header("4) Linha: Equipamento / Modelo / Nº de Série (PDF)")
...
sig_tec_up = st.file_uploader("Assinatura do técnico (PNG/JPG)", type=["png", "jpg", "jpeg"])

st.header("6) Cliente (rodapé do PDF)")
...
sig_cli_up = st.file_uploader("Assinatura do cliente (PNG/JPG)", type=["png", "jpg", "jpeg"])

# =========================
# CALIBRAÇÃO mínima (opcional)
# =========================
with st.expander("⚙️ Calibração fina (opcional)"):
    st.caption("A4 = 595 x 842 pt. Ajuste só se algo sair do lugar.")
...
def build_overlay_pdf(base_bytes: bytes) -> bytes:
    """Desenha os textos nas posições aproximadas e mescla na página 1 do PDF base."""
    packet = BytesIO()
    ...
    # DESCRIÇÃO — compõe texto final (SERIAIS + ATIVIDADE + INFO)
    bloco_desc = ""
    ...
    if sig_tec_up is not None:
        try:
            c.drawImage(ImageReader(sig_tec_up), x_tec + 310, y_tec - 10,
                        width=160, height=40, preserveAspectRatio=True, mask='auto')
        except Exception:
            pass

    # CLIENTE (rodapé: nome legível, RG/CPF, telefone, assinatura)
    ...
    if sig_cli_up is not None:
        try:
            c.drawImage(ImageReader(sig_cli_up), x_cli + 260, y_cli - 10,
                        width=220, height=60, preserveAspectRatio=True, mask='auto')
        except Exception:
            pass

    # Nº CHAMADO (inferior direito aprox.)
...
st.caption("Campos sem repetição. Se algo desalinha no PDF, ajuste a ⚙️ Calibração e me diga o deslocamento ideal para eu fixar no código.")
import streamlit as st
from datetime import date, time
from io import BytesIO
from PIL import Image
