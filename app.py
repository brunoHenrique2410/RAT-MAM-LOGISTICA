# app.py — RAT MAM (isolado por abas: Scanner | Assinaturas | Dados & PDF)
...
# ===== Estado =====
ss = st.session_state
ss.setdefault("scanned_items", [])           # lista de dicts {modelo,sn,mac,fonte}
ss.setdefault("photos_to_append", [])        # bytes JPEG
ss.setdefault("seen_hashes", set())          # deduplicação de imagens
ss.setdefault("seriais_texto", "")
ss.setdefault("anexar_fotos", True)

# assinaturas salvas (bytes PNG independentes do canvas)
ss.setdefault("sig_tec_png", None)
ss.setdefault("sig_cli_png", None)
ss.setdefault("assinatura_transparente", True)  # se False, salva com fundo branco chapado
    ...
    if zbar_decode:
        try:
            for obj in zbar_decode(pil):
                ...
                out.append((v, (obj.rect.left + obj.rect.width/2.0, obj.rect.top + obj.rect.height/2.0)))
        except Exception:
            pass
    if zxingcpp:
        try:
...
# ===== assinaturas (canvas -> PNG independente) =====
def signature_rgba_from_canvas(arr: np.ndarray, transparente: bool = True) -> Optional[Image.Image]:
    ...
    if transparente:
        ...
        # fundo branco chapado (sem alpha)
        out = np.full_like(rgba, 255, dtype=np.uint8)
        out[mask, :3] = 0
        out = out[:, :, :3]  # RGB
        return Image.fromarray(out, mode="RGB")

def png_bytes_from_signature_img(img: Image.Image, transparente: bool = True) -> Optional[bytes]:
    try:
        buf = BytesIO()
...
def scan_one_image(pil: Image.Image, fonte: str) -> Dict:
    ...
    if is_valid_sn(modelo, sn):
        ...
        if jpg:
            ...
            ss.seen_hashes.add(_fingerprint_bytes(jpg))
...
def add_scanned_item(item: Dict):
    ...
    keyset = {(e.get("modelo",""), e.get("sn",""), e.get("mac","")) for e in ss.scanned_items}
    if k not in keyset:
...
def insert_signature(page, label, png_bytes: Optional[bytes], rel_rect):
    ...
    page.insert_image(rect, stream=png_bytes)  # PNG mantêm alpha se houver
...
with tab1:
    ...
    with st.form("scanner_form"):
        ...
        with cbtn4:
            btn_push = st.form_submit_button("🡓 Jogar S/N no campo de seriais")
...
# -------- Aba 2: Assinaturas --------
with tab2:
    st.checkbox("Salvar assinatura com fundo transparente (recomendado)", key="assinatura_transparente", value=ss.assinatura_transparente)
    st.caption("Se seu visualizador de PDF escurecer o fundo, desmarque para salvar com fundo branco.")
    ...
    if st.button("💾 Salvar assinatura do TÉCNICO"):
        ...
        ss.sig_tec_png = png_bytes_from_signature_img(img, transparente=ss.assinatura_transparente) if img else None
        if ss.sig_tec_png:
            st.success("Assinatura do técnico salva.")
        else:
            st.warning("Nada para salvar (assine no quadro).")
    ...
    if st.button("💾 Salvar assinatura do CLIENTE"):
        ...
        img = signature_rgba_from_canvas(arr, transparente=ss.assinatura_transparente) if arr is not None else None
        ss.sig_cli_png = png_bytes_from_signature_img(img, transparente=ss.assinatura_transparente) if img else None
        if ss.sig_cli_png:
...
with tab3:
    ...
    st.subheader("3) Seriais (edite livremente)")
    ss.seriais_texto = st.text_area(
        "Seriais (um por linha) — use a aba Scanner para coletar e depois 'Jogar S/N'",
        value=ss.seriais_texto, height=200, key="seriais_texto_area"
    )

    st.subheader("4) Descrição de Atendimento")
    ...
    if st.button("🧾 Gerar PDF preenchido"):
        ...
        try:
            ...
            seriais = ss.get("seriais_texto_area", ss.get("seriais_texto",""))
            bloco = descricao_block(seriais, ss.get("atividade_txt",""), ss.get("info_txt",""))
            insert_descricao_autofit(page, ["DESCRIÇÃO DE ATENDIMENTO","DESCRICAO DE ATENDIMENTO"], bloco)

            # Assinaturas: usam PNG salvos — não dependem do canvas mais
            insert_signature(page, ["ASSINATURA:", "Assinatura:"], ss.get("sig_tec_png"), (110 - 2*CM, 0 - 1*CM, 330 - 2*CM, 54 - 1*CM))
            insert_signature(page, ["DATA CARIMBO / ASSINATURA", "ASSINATURA CLIENTE", "CLIENTE"], ss.get("sig_cli_png"), (110, 12 - 3.5*CM, 430, 94 - 3.5*CM))

            insert_right_of(page, [" Nº CHAMADO ", "Nº CHAMADO", "No CHAMADO"], ss.get("num_chamado",""), dx=-(2*CM), dy=10)

            # Anexar fotos
            if ss.anexar_fotos and ss.photos_to_append:
